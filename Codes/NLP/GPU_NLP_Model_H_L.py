...
M_rand = samples_dataframe_M.sample(n=1)
...
# In [11]:
batch_size = 64
model_name = 'NLP_' + str(num_random_rows) +'_64batchsize_2000steps_Data_MCC_ONLY_H_L'
...
count_dict[0] = 0
count_dict[1] = 0
# count_dict[2] = 0   ##Comment if doing 2 classes
...
for i in y_test2:
    if i==0:
        ...
        count_dict[1] += 1
    #elif i==2:             ##Comment if doing 2 classes
        #count_dict[2] += 1 ##Comment if doing 2 classes

# Print the counts of integers
for num, count in count_dict.items():
    print(f"The integer {num} appears {count} time(s) in the list.")
    
#n_High,n_Medium,n_Low = count_dict.values()  ##Comment if doing 2 classes
#count_High,count_Medium,count_Low = 0,0,0 ##Comment if doing 2 classes
...
for i in range(len(prediction)):
    if prediction[i] == y_test2[i] and prediction[i] == 0:
        count_High += 1
    #elif prediction[i] == y_test2[i] and prediction[i] == 1: ##Comment if doing 2 classes
        #count_Medium += 1 ##Comment if doing 2 classes
    elif prediction[i] == y_test2[i] and prediction[i] == 1:
        count_Low += 1

Correct_High = count_High/n_High * 100
#Correct_Medium = count_Medium/n_Medium * 100  ##Comment if doing 2 classes
Correct_Low = count_Low/n_Low * 100

results_RF2 = {'Correctly Predicted-High': Correct_High,
           #'Correctly Predicted-Medium': Correct_Medium, \  ##Comment if doing 2 classes
