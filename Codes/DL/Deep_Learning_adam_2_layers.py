...
input_X = input_X.loc[1]
input_X.to_csv('DL Results_2layers_adam_Removed_N' + '_cols_OHE.csv', index=False)
...
plt.savefig('DL Results_2layers_adam_Removed_N_Accuracy & Loss Plot.jpg')
plt.show()
...
with open('DL Results_2layers_adam_Removed_N', 'w') as f:
    for key, value in results_RF2.items(): 
        f.write('%s:%s\n' % (key, value))
f.close()

### Saving the Model ###
full_file_name = 'DL Results_2layers_adam_Removed_N' + '.h5'
